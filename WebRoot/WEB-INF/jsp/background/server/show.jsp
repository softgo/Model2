...
.block_icon_disk {
	text-align: center;
	background:
		url(<%=request.getContextPath()%>/images/icons/other/disk.jpg)
		no-repeat;
}
...
.dhooo_tab {
	...
	margin-right: 1px;
	background: #fff
		url(<%=request.getContextPath()%>/images/main_bg.gif) repeat-x 0
		100%;
...
.tab_btn {
	...
	padding-left: 20px;
	padding-top: 5px;
	background: url(<%=request.getContextPath()%>/images/tabbar.gif)
		repeat-x;
}
...
<script type="text/javascript">
	...
	function loadTabData() {
		$.ajax({
			type : 'POST',
			url : "<%=request.getContextPath()%>/background/serverInfo/info.html",
			dataType : 'json',
	...
	function createDiskInfosHtml(type, datas) {
	 ...
	 function fchart(name,pen,label1,value1,label2,value2,charId,divId){
		  ...
		  var chart = new FusionCharts("<%=request.getContextPath()%>/FusionCharts/Pie3D.swf", charId, "400", "250");
