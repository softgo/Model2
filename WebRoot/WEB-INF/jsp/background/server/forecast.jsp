...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/jquery.showLoading.js"></script>
...
.block_icon_disk{text-align:center; background:url(<%=request.getContextPath()%>/images/icons/other/disk.jpg) no-repeat;}
.block_icon_cpuInfos{text-align:center; background:url(<%=request.getContextPath()%>/images/icons/other/disk.jpg) no-repeat;}
...
$(function(){
	$(".loading").showLoading(); 
	var url = "<%=request.getContextPath()%>/background/serverInfo/warnInfo.html";
	$.ajax({
		type : 'POST',
		url : url,
		dataType : 'json',
		success : function(data) {
		 var tbodyContent="";
		 	...
		 	tbodyContent = tbodyContent + "<td>使用率超出 <input class='inputclass' name='cpu' id='cpu' type='text' value='"+data.cpu+"'/> %,发送邮箱提示 <input type='button' value='更新' id='btnLoadData' class='input_btn_style1' onclick='modifySer(\"cpu\");'/></td>"+
		 	"<td rowspan='3'><input class='inputclass'style='width:180px;height:20px;' name='toEmail' id='toEmail' type='text' value='"+data.toEmail+"'/><input type='button' value='更新' id='btnLoadData' class='input_btn_style1'  onclick='modifySer(\"toEmail\");'/></td>";
		 	 tbodyContent = tbodyContent + "</tr>";
		 	tbodyContent = tbodyContent + "<tr>";
...
function modifySer(key){
	$.ajax({
        async: false,
        url: "<%=request.getContextPath()%>/background/serverInfo/modifySer.html",
 ...
 function fchart(name,pen,label1,value1,label2,value2,charId,divId){
	  ...
	  var chart = new FusionCharts("<%=request.getContextPath()%>/FusionCharts/Pie3D.swf", charId, "350", "200");
